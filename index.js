...
async function startNaze() {
    ...
    // Get Profile Picture Group
       ...
       naze.send5ButImg(pea[0].id, `*Group Settings Change* \n\nGroup has been closed by admin, Now only admin can send messages !`, `Group Settings Change `, wm_nazedev, [])
        } else if(pea[0].announce == false) {
        naze.send5ButImg(pea[0].id, `「 *Group Settings Change* \n\nGroup has been opened by admin, Now participants can send messages !`, `Group Settings Change`, wm_nazedev, []  )
        } else if (pea[0].restrict == true) {
        naze.send5ButImg(pea[0].id, `「 *Group Settings Change* \n\nGroup info has been restricted, Now only admin can edit group info !`, `Group Settings Change `, wm_nazedev,  [])
        } else if (pea[0].restrict == false) {
        naze.send5ButImg(pea[0].id, `「 *Group Settings Change* \n\nGroup info has been opened, Now participants can edit group info !`, `Group Settings Change`, wm_nazedev, []  )
        } else {
        naze.send5ButImg(pea[0].id, `「 *Group Settings Change* \n\nGroup Subject has been changed to *${pea[0].subject}*`, `Group Settings Change `,  wm_nazedev, [])
      }
     })
